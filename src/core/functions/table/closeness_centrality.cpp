#include "duckpgq/core/functions/table/closeness_centrality.hpp"
...
unique_ptr<TableRef>
ClosenessCentralityFunction::ClosenessCentralityBindReplace(ClientContext &context, TableFunctionBindInput &input) {
	...
	auto select_node = CreateSelectNode(edge_pg_entry, "closeness_centrality", "closeness_centrality");
	...
	result->alias = "closeness_centrality";
...
// Register functions
//------------------------------------------------------------------------------
void CoreTableFunctions::RegisterClosenessCentralityTableFunction(ExtensionLoader &loader) {
	loader.RegisterFunction(ClosenessCentralityFunction());
}
