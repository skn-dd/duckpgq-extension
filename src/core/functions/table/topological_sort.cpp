#include "duckpgq/core/functions/table/topological_sort.hpp"
...
unique_ptr<TableRef> TopologicalSortFunction::TopologicalSortBindReplace(ClientContext &context,
	...
	auto select_node = CreateSelectNode(edge_pg_entry, "topological_sort", "topological_order");
...
// Register functions
//------------------------------------------------------------------------------
void CoreTableFunctions::RegisterTopologicalSortTableFunction(ExtensionLoader &loader) {
	loader.RegisterFunction(TopologicalSortFunction());
}
