...
static void CosineSimilarityFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	auto &func_expr = state.expr.Cast<BoundFunctionExpression>();
	auto &info = func_expr.bind_info->Cast<CosineSimilarityFunctionData>();
	...
	for (idx_t i = 0; i < args.size(); i++) {
		...
		double denom = std::sqrt(static_cast<double>(na.size())) * std::sqrt(static_cast<double>(nb.size()));
		result_data[i] = (denom > 0) ? static_cast<double>(intersection) / denom : 0.0;
	}
