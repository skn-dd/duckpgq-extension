...
static void OverlapSimilarityFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	auto &func_expr = state.expr.Cast<BoundFunctionExpression>();
	auto &info = func_expr.bind_info->Cast<OverlapSimilarityFunctionData>();
	...
	if (!(csr_entry->second->initialized_v && csr_entry->second->initialized_e)) {
		throw ConstraintException("Need to initialize CSR before computing overlap similarity.");
	}
	...
	for (idx_t i = 0; i < args.size(); i++) {
		...
		int64_t min_size = static_cast<int64_t>(std::min(na.size(), nb.size()));
		result_data[i] = (min_size > 0) ? static_cast<double>(intersection) / static_cast<double>(min_size) : 0.0;
	}
...
// Register functions
//------------------------------------------------------------------------------
void CoreScalarFunctions::RegisterOverlapSimilarityScalarFunction(ExtensionLoader &loader) {
	loader.RegisterFunction(ScalarFunction("overlap_similarity",
	                                       {LogicalType::INTEGER, LogicalType::BIGINT, LogicalType::BIGINT},
	                                       LogicalType::DOUBLE, OverlapSimilarityFunction,
	                                       OverlapSimilarityFunctionData::OverlapSimilarityBind));
}
